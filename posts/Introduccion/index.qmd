...
Map.addLayer(country, {palette: 'red'}, 'Paises', 0.5);
...
```
Otro de los objetos importantes dentro de GEE son las colecciones de imágenes, en una collección de se encuentran las imágenes en donde todas comparten ciertas propiedades comunes como son su sistema de referencia, el algorithmo con el que se hizo, etc. 

Tomar la colección de imagenes completa en general no es necesario, por lo general o se selecciona una o se obtiene sus agragados dependiendo de la banda. Las imagenes satelitales en general contienen muchas bandas de los distintos sensores o imagenes generadas a través de algoritmos usando los datos de sensores satelitales. Como no es necesario todas las bandas ni todas las imagenes es necesario tener la capacidad de poder seleccionar lo que es de nuestro interes, en GEE existen objetos que nos ayudan a seleccionar lo que es de nuestro interes, estos objetos son los filtros [`ee.Filter`] (https://developers.google.com/earth-engine/apidocs/ee-filter), estos filtros pueden filtrar campos en especifico. 

En la siguiente linea se hacen dos objetos de tipo `ee.Filter` que filtran las imagenes de la coleccion de imagenes usando las fechas, en las siguentes lineas de código en las variables `no2_18` y `no2_19` se almacenan los resultados de las mismas como podemos observar en la consola se van a imprimir.

Para poder hacer una visualización de las imágenes, es necesario dar un conjunto de parámetros al objeto donde esta nuestro mapa [`Map`](https://developers.google.com/earth-engine/apidocs/map-addlayer). En el caso anterior donde se muestra el poligono en el mapa no se añade ningun color pero en este se cambio a rojo y se añade el nombre de la capa. Para hacer la visualización de las imágenes es necesario conocer sus valores máximos y mínimos, adicionammente se le da un conjunto de colores para al añadirse al mapa estos se cuanticen. 
